...
class Give_Item_List extends Component {
  ...
  setCurrentItemsForState = (items) => {
    ...
    displayedItems = Object.keys(items).slice(
      this.state.offset,
  ...
  async componentDidMount() {
    ...
    if (itemsForState !== 'そのカテゴリーに分類する商品はありません') {
        ...
        pickedGiveItems.map(async (giveItem) => {
          await axios
            ...
            .then((res) => {
              itemsForState = {
                ...itemsForState,
                [res.data.id]: { ...res.data, doneDeal: giveItem.doneDeal },
              };
            })
            .catch((err) => console.log(err));
        })
      ...
      // ページネーションのページ数を決定
      this.setState({
        pageCount: Math.ceil(Object.keys(itemsForState).length) / this.state.perPage,
      });
  ...
  render() {
    ...
    if (this.state.allItems === 'そのカテゴリーに分類する商品はありません') {
      itemCards = (
        <NotHaveText position_left={this.props.position_left}>{this.state.allItems}</NotHaveText>
      );
    } else {
      itemCards = Object.keys(this.state.currentItems).map((parentId, idx) => {
        return (
          <ItemCard
            ...
            doneDeal={this.state.currentItems[parentId]['doneDeal']}
          />
        );
      });
